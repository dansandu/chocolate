...
static void drawFlatTriangle(const std::vector<Vector3>& vertices, const std::vector<Point2>& points,
                             const std::vector<int>& permutation, const bool wireframe, const bool drawFlatEdge,
                             const std::function<void(const ConstantVector3View vertex)>& callback)
{
                   ...
                   const bool wireframe, const std::function<void(const ConstantVector3View vertex)>& callback)
{
        ...
        points[permutation[1]].y() != points[permutation[2]].y())
    {
        ...
        drawFlatTriangle(vertices, points, {permutation[0], permutation[1], permutation[2]}, wireframe, drawFlatEdge,
                         callback);
        drawFlatTriangle(vertices, points, {permutation[3], permutation[1], permutation[2]}, wireframe, drawFlatEdge,
                         callback);
    }
    else if (points[permutation[0]].y() == points[permutation[1]].y())
    {
        ...
        drawFlatTriangle(vertices, points, {permutation[2], permutation[0], permutation[1]}, wireframe, drawFlatEdge,
                         callback);
    }
    else
    {
