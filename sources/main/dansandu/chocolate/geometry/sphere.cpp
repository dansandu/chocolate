...
std::pair<Vertices, Triangles> generateSphere(const double radius, const int yResolution, const int zResolution)
{
    ...
    for (auto j = 0; j < yResolution; ++j)
    {
        ...
        const auto yRadians = 2.0 * j * pi<double> / yResolution;

        for (auto k = 0; k < zResolution; ++k)
        {
            const auto vertex = k * yResolution + j + 1;
            const auto zRadians = (k + 1) * pi<double> / (zResolution + 1) + 0.5 * pi<double>;
    ...
    vertices(vertexCount - 1, 0) = 0.0;
    vertices(vertexCount - 1, 1) = -radius;
    vertices(vertexCount - 1, 2) = 0.0;
    vertices(vertexCount - 1, 3) = 1.0;
